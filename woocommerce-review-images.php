...
if ( ! class_exists( 'WC_Review_Images_Simplified' ) ) {
    class WC_Review_Images_Simplified {
        ...
        public function display_upload_field_and_nonce() {
            ...
            echo '<label for="review_image_upload">' . esc_html__( 'Upload an image (optional, max 2MB, JPG, PNG, GIF)', 'woocommerce-review-images' ) . '</label>';
        ...
        public function handle_image_upload( $commentdata ) {
            ...
            if ( isset( $_FILES['review_image_upload'] ) && !empty( $_FILES['review_image_upload']['name'] ) && $_FILES['review_image_upload']['error'] == UPLOAD_ERR_OK ) {
                ...
                if ( $file['size'] > self::MAX_FILE_SIZE_BYTES ) {
                    error_log( 'WCRI Error: Uploaded file exceeds max size limit. File: ' . $file['name'] );
                    return $commentdata; 
                }
                ...
                if ( ! $file_type['ext'] || ! $file_type['type'] ) {
                    error_log( 'WCRI Error: Uploaded file type is not allowed. File: ' . $file['name'] . ' Detected type: ' . $file['type'] );
                    return $commentdata; 
                }
                ...
                if ( is_wp_error( $attachment_id ) ) {
                    error_log( 'WCRI Error: media_handle_upload failed. Message: ' . $attachment_id->get_error_message() . ' File: ' . $file['name'] );
                } else {
